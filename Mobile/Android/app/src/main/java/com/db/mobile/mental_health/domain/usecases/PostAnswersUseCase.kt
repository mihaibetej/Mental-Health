...
class PostAnswersUseCase @Inject constructor(private val dataSource: SurveyDataSource) {

    suspend fun execute(questions: List<SurveyQuestion>): OpResult<Int, NoDataException> {
        ...
        return when (dataSource.postAnswers(answerData)) {
            is Success -> {
                Success(questions.sumBy { it.answer })
            }
            is Failure -> {
